...
class ImbPrometheus:
    ...
    async def select_endpoints(self, call_next, state_data):
        if not state_data:
            ...
            if self.k8sImb.prometheusService:
                ...
                try:
                    requests.get(state_data['local_endpoint'])
            ...
            # prompt for endpoint
            state_data['prometheus_endpoint'], state_data['local_endpoint'] = await self.ui.prompt_text_input(
            ...
            if state_data['prometheus_endpoint'] is None or state_data['local_endpoint'] is None:
                return None
    ...
    async def prompt_port_forward(self, call_next, state_data):
        if not state_data or not state_data.get('port_forward_accepted'): # Ensure disclaimer accepted when skipping prompt
            state_data = {
                'interacted': False, # only prompting for accept here so skip over if going back
        ...
        if not state_data:
            ...
            if len(matching_known_metrics) == 1:
                ...
                if desired_metric_indexes is None:
                    return None
                state_data['desired_deployment_metrics'] = [matching_known_metrics[i] for i in desired_metric_indexes]
            else:
                state_data['interacted'] = True
                ...
                if desired_metric_indexes is None:
                    return None
                state_data['desired_deployment_metrics'] = [ req_metrics[i] for i in desired_metric_indexes ]
    ...
    async def configure_deployment_metrics(self, call_next, state_data):
        if not state_data:
            ...
            while(i < num_metrics):
                ...
                state_data['configured_deployment_metrics'][perf_name] = { 'query': query_text }
                if perf_unit:
    ...
    async def select_perf(self, call_next, state_data):
        if not state_data:
            ...
            if metric_names:
                ...
                if desired_index is None:
                    return None
                state_data['perf_metric'] = "metrics['{}']".format(metric_names[desired_index])
            else:
